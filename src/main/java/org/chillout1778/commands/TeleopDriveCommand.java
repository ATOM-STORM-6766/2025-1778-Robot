...
public class TeleopDriveCommand extends Command {
            ...
            speeds = new ChassisSpeeds(
                    ...
                    fixRotationInput(
                            turnPID.calculate(Swerve.getInstance().getEstimatedPose().getRotation().getRadians(),
                                    Math.abs(Math.PI / 2
                                            - Swerve.getInstance().getEstimatedPose().getRotation().getRadians()) < Math
                                                    .abs(-Math.PI / 2 - Swerve.getInstance().getEstimatedPose()
    ...
    private boolean isWithinAlignmentTolerance(Pose2d currentPose) {
        ...
        // For a complete implementation, you would compare current pose with target
        // pose
