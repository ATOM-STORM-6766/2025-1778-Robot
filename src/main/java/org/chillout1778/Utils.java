...
public class Utils {
  ...
  public static boolean isInsideField(Translation2d translation) {
    return translation.getX() > 0.0
        && translation.getY() > 0.0
  ...
  public static Translation2d mirrorIfRed(Translation2d translation) {
    return Robot.getInstance().isRedAlliance() ? mirror(translation) : translation;
  }

  public static Rotation2d mirrorIfRed(Rotation2d rotation) {
    return Robot.getInstance().isRedAlliance() ? mirror(rotation) : rotation;
  }

  public static Pose2d mirrorIfRed(Pose2d pose) {
    return Robot.getInstance().isRedAlliance() ? mirror(pose) : pose;
  }

  public static void addClosedLoopProperties(String name, TalonFX motor, SendableBuilder builder) {
    builder.addDoubleProperty(
        name + " voltage (V)", () -> motor.getMotorVoltage().getValueAsDouble(), (value) -> {});
    ...
    builder.addDoubleProperty(
        name + " raw velocity", () -> motor.getVelocity().getValueAsDouble(), (value) -> {});
    builder.addDoubleProperty(
