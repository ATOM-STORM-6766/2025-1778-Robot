...
public class SwerveModule implements Sendable {
  ...
  public SwerveModule(
    ...
    driveConfig.Feedback =
        new FeedbackConfigs().withSensorToMechanismRatio(Constants.Swerve.DRIVE_RATIO);
    ...
    turnConfig.Feedback =
        new FeedbackConfigs().withSensorToMechanismRatio(Constants.Swerve.TURN_RATIO);
  ...
  public void driveState(SwerveModuleState state) {
    state.optimize(Rotation2d.fromRadians(getTurnPosition()));
    double goalTurnPosition = state.angle.getRadians();
    double goalDriveVelocity =
        state.speedMetersPerSecond * Math.cos(getTurnPosition() - goalTurnPosition);
    ...
    commandedVolts =
        driveFeedforward.calculateWithVelocities(goalDriveVelocity, getDriveAcceleration());
  ...
  public void initSendable(SendableBuilder builder) {
    ...
    builder.addDoubleProperty(
        "raw cancoder position", () -> canCoder.getAbsolutePosition().getValueAsDouble(), null);
    ...
    builder.addDoubleProperty(
        "drive stator current", () -> driveMotor.getStatorCurrent().getValueAsDouble(), null);
