...
public class Arm extends SubsystemBase {
    ...
    public enum RollerState {
        ...
        public final double dutyCycle;        RollerState(double dutyCycle) {
    ...
    public enum Side { Left, Right, Neither }    private final CANcoder absoluteEncoder;
    ...
    private Arm() {absoluteEncoder = new CANcoder(Constants.CanIds.ARM_ENCODER);
    ...
    private double getCloseClampedPosition() {
        double x = absoluteEncoder.getPosition().getValueAsDouble() - Constants.Arm.PIVOT_ABS_ENCODER_OFFSET_ENCODER_ROTATIONS;
        while (x < -0.5) x += 1.0;
        while (x > 0.5) x -= 1.0;
